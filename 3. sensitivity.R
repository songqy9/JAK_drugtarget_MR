...
mr_presso <- read.csv("result_presso_ukb-b-8714.csv",header = T) %>% dplyr::select(MR.PRESSO.results.Global.Test.Pvalue)
# IVW
...
write.csv(sensitivity_table,"tidy_reasult/IVW_sensitivitytable.csv",row.names = F,quote = F)
# smr 手动改
write.csv(result_smr,"tidy_reasult/smr_sensitivitytable.csv",row.names = F,quote = F)
